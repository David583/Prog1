...
int main()
{
	...
	Function s (one, rmin + 2, rmax - 3, origo, n_points + 100, xscale, yscale);
	Function sq (square, rmin - 1, rmax - 2, origo, n_points, xscale, yscale);
	Function cos_func ( [] (double x) { return cos(x); },
						rmin * 2, rmax / 2, origo, n_points, xscale, yscale);

	Axis x {Axis::x, Point{20, y_orig}, xlength, xlength/xscale, "x"};
	Axis y {Axis::y, Point{x_orig, ylength + 20}, ylength, ylength/yscale, "y"};

	Rectangle r {Point{250,100}, 100, 50};

	r.set_fill_color(Color::blue);
	r.set_style(Line_style(Line_style::dot, 4));

	Text t {Point{200,400}, "Hello proba!"};
	t.set_font(Font::times_bold);
	t.set_font_size(24);

	Image ii {Point{150,150}, "badge.jpg"};

	Circle c {Point{700,700}, 50};
